...
## Introduction
Sometimes you have to work with IFC files without 3d rendering, when you just need to extract spatial informations, spatial tree,  project infos etc...  
...
const ifcFileLocation = "duplex-apartment.ifc";
let modelID = 0;
...
Okay, now i want to get all IFCSPACES and display them somewhere. Lets make a function wich grab all IFCSPACE.
...
If your attentive you will notice the const IFSPACE from ``` js let lines = ifcapi.GetLineIDsWithType(modelID, IFCSPACE); ``` it means I want to get all IFCSPACE from my ifc file. Don't forget to import this as follow.
